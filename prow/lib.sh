...
function build_images() {
  ...
  nonDistrolessTargets="docker.app docker.app_sidecar_ubuntu_xenial docker.ext-authz "
  if [[ "${JOB_TYPE:-presubmit}" == "postsubmit" ]]; then
    # We run tests across all VM types only in postsubmit
    nonDistrolessTargets+="docker.app_sidecar_ubuntu_xenial docker.app_sidecar_debian_10  docker.app_sidecar_centos_7 "
