...
func (r *resolver) LookupIP(dnsName string) []string {
	...
	return watch.answers // read-only snapshot
}
...
func (r *resolver) AddUpdateHandler(handler UpdateHandler) {
	...
	r.handlers = append(append([]UpdateHandler{}, r.handlers...), handler) // copy-on-write
}

func (r *resolver) getUpdateHandlers() []UpdateHandler {
	r.mu.RLock()
	defer r.mu.RUnlock()

	return r.handlers // read-only snapshot
}

func (r *resolver) waitUpdates() {
	for update := range r.updateCh {
		answersChanged, answers := r.updateAnswers(update)
		if answersChanged {
			r.notifyHandlers(update.dnsName, answers)
		}
	}
}

func (r *resolver) updateAnswers(update dnsUpdate) (answersChanged bool, answers []string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	return !NewNameSet(prev...).Equal(NewNameSet(next...)), next
}

func (r *resolver) notifyHandlers(dnsName string, answers []string) {
	handlers := r.getUpdateHandlers()
	log.Debugf("Notifying %d handlers that DNS name %q now resolves into a different set of IP addresses: %v",
		len(handlers), dnsName, answers)
