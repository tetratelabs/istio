...
func (r *resolver) waitUpdates() {
	for update := range r.updateCh {
		...
		old := watch.answers
		new := make([]string, len(update.addresses))
		...
		if NewNameSet(old...).Equal(NewNameSet(new...)) {
			continue			
		}

		r.mu.RLock()
		log.Debugf("Notifying %d handlers that DNS name %q now resolves into a different set of IP addresses: %v", len(r.handlers), update.dnsName, new)
...
func watchDnsName(dnsName string, stopCh <-chan struct{}, client Client, updateCh chan<- dnsUpdate){
	...
	for {
		...
		if err != nil {
			delayTillRefresh = delayTillRefresh / 2
		...
		log.Debugf("Resolved DNS name %q into %v, effective TTL == %s, will repeat DNS resolution in %s", dnsName, addresses, effectiveTTL, delayTillRefresh)
