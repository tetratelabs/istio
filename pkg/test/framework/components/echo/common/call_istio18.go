...
func callInternalIstio18(srcName string, opts *echo.CallOptions, send sendFunc,
	doRetry bool, retryOptions ...retry.Option) (client.ParsedResponses, error) {

	if err := fillInCallOptionsIstio18(opts); err != nil {
		return nil, err
	}

	// Forward a request from 'this' service to the destination service.
	port := opts.Port.ServicePort
	addressAndPort := net.JoinHostPort(opts.Host, strconv.Itoa(port))
...
func CallEchoIstio18(opts *echo.CallOptions, retry bool, retryOptions ...retry.Option) (client.ParsedResponses, error) {
	...
	return callInternalIstio18("TestRunner", opts, send, retry, retryOptions...)
}

func ForwardEchoIstio18(srcName string, c *client.Instance, opts *echo.CallOptions,
	retry bool, retryOptions ...retry.Option) (client.ParsedResponses, error) {
	return callInternalIstio18(srcName, opts, func(req *proto.ForwardEchoRequest) (client.ParsedResponses, error) {
		return c.ForwardEcho(context.Background(), req)
	}, retry, retryOptions...)
}

func fillInCallOptionsIstio18(opts *echo.CallOptions) error {
	if opts.Target != nil {
	...
	} else if opts.Port == nil || opts.Port.ServicePort == 0 || (opts.Port.Protocol == "" && opts.Scheme == "") || opts.Host == "" {
	...
	if h := opts.Headers["Host"]; len(h) == 0 && opts.Target != nil {
		...
		opts.Headers["Host"] = []string{opts.Target.Config().FQDN()}
	}
