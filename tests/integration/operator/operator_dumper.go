...
func (d *operatorDumper) Dump(ctx resource.Context) {
	...
	kube2.DumpPods(ctx, dir, "istio-system", []string{"istio=pilot"})          // dump pilot pods for debugging
	kube2.DumpPods(ctx, dir, "istio-system", []string{"istio=ingressgateway"}) // dump ingressgateway pods for debugging
}
