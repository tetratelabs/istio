...
func (c *Controller) refreshGatewayEndpoints(dnsName string) {
	if c.isGatewayDNS(dnsName) {
		log.Debugf("Triggering a full xDS push since DNS name %q of a network gateway is now resolved into a different set of IP addresses: %v", dnsName, c.dnsResolver.LookupIP(dnsName))
